...
export default function DataGrid(props:any) {
  ...
  const [ignoreFields] = useState(props.ignoreFields === undefined ? [] : props.ignoreFields)
...
function generateRandomString(length: number) {
    ...
    if (allDatas) {
      ...
      if (allDatas.length > 0) {
        if (!allDatas[0]?.hasOwnProperty(primaryKey)) {
          ...
          if (allDatas[0])
            setPrimaryKey(Object.keys(allDatas[0])[0])
        }
      }
    }
  }
  ...
  function getPaging() {
    ...
    let filteredDataLenght = allDatas.slice().filter(getFilteredDatasForSearchParams).length;
  ...
  function getPagingButtons(pages: number) {
    ...
    let prevButton = currentPage !== 0 ? <button key={key} id={key} className="paging-button" onClick={() => goToPrevPage()}>
      {prevSymbol}
    </button> : <button key={key} id={key} className="paging-button" disabled >
      {prevSymbol}
    </button>
    ...
    if (currentPage > maxPagesDifference) {
      buttons.push(
        <button className="paging-button" key={"first_page_button"} onClick={() => setCurrentPage(0)}>
          1
        </button>
      )
      buttons.push(<button disabled className="paging-button">..</button>)
    }
    ...
    for (let i = 0; i <= pages; i++) {
      ...
      if (showButton) {
        ...
        let button = currentPage !== i ? <button key={key} id={key}
    ...
    let nextButton = currentPage < pages - 1 ? <button key={key} id={key} className="paging-button" onClick={() => goToNextPage()}>
      {nextSymbol}
    </button> : <button key={key} id={key} className="paging-button" disabled >
      {nextSymbol}
    </button>
    ...
    if (pages > currentPage + maxPagesDifference) {
      buttons.push(<button disabled className="paging-button">..</button>)
      buttons.push(
        <button key={"last_page"} id={key} className="paging-button" onClick={() => setCurrentPage(pages)}>{pages + 1}</button>
      );
    } else {
      <button key={"page_" + pages} id={key} className="paging-button" onClick={() => setCurrentPage(pages)}>{pages + 1}</button>
    }
    buttons.push(
      nextButton
    );
              ...
              if (a !== undefined)
                if (a[curSortingField] !== undefined) {
                  ...
                  if (columns[curSortingField]?.overrideSortingValueFunction) {
                    valueA = columns[curSortingField]?.overrideSortingValueFunction(a)
                    useFunction = true;
                  }

                  let valueB: (any) = b[curSortingField]
                  if (useFunction) {
                    valueB = columns[curSortingField]?.overrideSortingValueFunction(b)
                  }
  ...
  function getTable() {
    ...
    for (var iii = 0; iii < currentDatas.length; iii++) {
      ...
      for (var ff in curItem) {
        ...
        if (getShowColumn(ff)) {
          ...
          if (allowSelection) {
            ...
            rowCells.push(
              <td id={key} className={classes}>{fieldValue}</td>
            )
          }
          columnCount++
        }
      ...
      if (commandButtons !== undefined) {
        for (var j = 0; j < commandButtons.length; j++) {
          ...
          //key = UtilityFunctions.tryUniquifyKey(key)
          rowCells.push(
            <td key={key} id={key} className="last-cell">{commandButtonsArray}</td>
          );
    ...
    let gridTitle = <div className="grid-title">
      {text}   {collapsed}</div>
    ...
    //key = UtilityFunctions.tryUniquifyKey(key)
    let containerClasses = "grid-container " + (canCollapse ? (collapsed ? " collapse " : " not-collapsed ") : "")
    if (props.containerClass)
      containerClasses += props.containerClass
  ...
  function getEmptyTable(loading:boolean) {
    ...
    let key = "dataGridTr" + elementIndex
    let containerClasses = "grid-container " + (collapsed ? " collapse " : " not-collapsed ")
  ...
  function getCollapseButton() {
    ...
    return canCollapse ? <div id={"collapse-button"} className="no-border-button top-right-button" onClick={() => setCollapsed(!collapsed)}>{symbol}</div> : <></>
  }
      ...
      if (allDatas[0] !== undefined) {
        ...
        for (var f in curItem) {
          if (field === f) {
            ...
            let nextField = keys[pos + 1] !== undefined ? keys[pos + 1] : undefined;
  ...
  function getRowsDatas(item: any) {
    ...
    try {
      ...
      let found = rowsDatas.find((element: any) => element?.primaryKeyValue === itemKey);
      if (found) {
        let rowDatas = found
        return rowDatas;
      } else {
  ...
  function getGlobalCommandButtonsSection() {
    ...
    for (var b in globalCommandButtons) {
      ...
      let returnValue = curButton.returnType === "selected" ?
        getSelectedRowsDatas() : undefined
    ...
    return (
      <div className="global-command-buttons-section">{buttons}</div>)
  }
  ...
  function getColumnHeaderLabel(field: any) {
    return props?.columns[field]?.label ? props.columns[field].label : field.charAt(0).toUpperCase() + field.slice(1)
  }
  ...
  function getOtherHeaderContent(f: any) {
    let headerContent: any = <div className="top-down-group">
      <div>{f.label}</div>
      {f.headerContent}
      <></>
    </div>
    ...
    let style = {
      maxWidth: otherColumns[f]?.maxWidth ? otherColumns[f]?.maxWidth : "inherit"
    }

    let key = "dataGridHeaderField_" + f.index + f.label + elementIndex

    let headerField = (
      <th key={key} id={key} className={classes} style={style} onClick={() => getHeaderCellOnClick(f)} >{headerContent}</th>
    )

    return headerField;
  }

  function getColumnOtherColumnCellContent(curItem: any, count:number, field: any) {
    let fieldValue = field.getContentFunction ? field.getContentFunction(curItem) : field.content ? field.content : field.getValueFunction ? field.getValueFunction(curItem) : "NO VALUE"
    ...
    return (<td key={key} id={key} className={classes} style={style}>{fieldValue}</td>)
  }
  // Other columns UP ***************************
  ...
  function getColumnHeaderContent(field: any) {
    ...
    columnName = getColumnHeaderLabel(field) + " " + getSortingIconForColumn(field);
    ...
    if (currentDatas) {
      headerFieldContent = (
        <div className="top-down-group">
          ...
          {getAllowSearchForColumn(field) ? <input placeholder="Rechercher.." onChange={(x) => updateSearchField(field, x.target.value)} className="column-search" ></input > : <></>}
        </div>
      )
    } else {
      headerFieldContent = (
    ...
    let cellField = (
      <th key={key} id={key} className={classes} style={style} >{headerFieldContent}</th>
    )
  ...
  function getAllowSearchForColumn(field: any) {
    ...
    let allowColumnSearch = columns[field] ?
      columns[field].allowSearch !== undefined ? columns[field].allowSearch : allowAllSearch : allowAllSearch;
  ...
  function getDataCellValue(currentRow: any, value: any, field: any) {
