...
export default function DataGrid(props: IDatagridProps) {
  ...
  const [rowsData, setRowsData]: any = useState(undefined)
  ...
  useLayoutEffect(() => {
    if (!initialized) {
      ...
      if (allData) setInitialized(true)
    }
  ...
  function initializeRowsDatas() {
    let allDatasRows = rowsData
    allDatasRows = []

    if (allData?.length > 0) {
      for (let i = 0; i < allData.length; i++) {
        if (!allData[i] || !allData[i]['rowID']) {
          break
        }
  ...
  if (!props.isLoading && allData && allData?.length > 0) {
    return <>{getTable()}</>
  } else {
  ...
  function getPaging() {
    ...
    let slicedAllData = allData?.slice()
  ...
  function updateCurrentDatas() {
    ...
    if (allData && !rowsData) initializeRowsDatas()
  ...
  function getTable() {
    let headerCells = []
    if (!currentData && allData) updateCurrentDatas()
    if (!currentData) return <></>

    // Selection **
    let curItem = allData[0]
    ...
    for (var iii = 0; iii < currentData.length; iii++) {
      let rowCells = []
      let curRow: any = currentData[iii]
    ...
    if (props.getGridFooterContent) {
      let datas = { allDatas: allData, currentDatas: currentData }
  ...
  function getRowsDatas(row: any) {
    if (!rowsData || rowsData.length === 0 || !row) return

    if (rowsData.length <= 0) return

    try {
      let itemKey = row['rowID']
      let found = rowsData.find((element: any) => element?.rowID === itemKey)
      if (found) {
        let rowDatas = found
        return rowDatas
      } else {
  ...
  function onEditButtonClick(row: any) {
    ...
    if (row) {
      ...
      let rowData = rowsData.find((element: any) => element.rowID === itemKey)
      if (rowData) {
        rowData.inEditMode = !rowData.inEditMode
        // in rowsDatas, replace the item with the same rowID as rowData by rowData
        let index = rowsData.indexOf(
          rowsData.find((x: any) => x.rowID === rowData.rowID)
        )
  ...
  function onRowCellClick(e: any, row: any, field?: any) {
    ...
    if (row) {
      ...
      if (props.singleRowSelection) {
        let selectedRows = rowsData.filter(
  ...
  function getSelectedRowsDatas() {
    ...
    let selectedRows = rowsData?.filter((x: any) => x?.selected)
    let datas: any = []
